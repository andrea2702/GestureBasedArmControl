...
def main(fps_cap=60, show_fps=True, source=0):
    ...
    gripper = rospy.Publisher('/franka_gripper/move/goal',
                          MoveActionGoal, queue_size=1, tcp_nodelay=True)
    ...
    while not rospy.is_shutdown():
        ...
        if delta_time >= time_lim:  # if the time passed is bigger or equal than the frame time, process the frame
            ...
            frame, handness_list = HandDet.findHands(frame=frame, return_handedness=True)
            ...
            try:
                ...
                try:
                    hand_lmlist_2, frame = HandDet.findHandPosition(
                        frame=frame, hand_num=1, draw=False)
                except:
                    print("No hand 2")

                # If all the keypoints data is available, compute the pose angles, the hand aperture and send it to the robot
                if len(lm_list_pose) > 0 and len(lm_3dlist_pose) > 0 and len(hand_lmlist) > 0 and len(hand_lmlist_2) > 0:
                    ...
                    frame, aperture = HandDet.findHandAperture(
                        frame=frame, lm_list = hand_lmlist, verbose=True, show_aperture=True)
                    
                    if(aperture < 50):
                        aperture = 0
                    
                    frame, aperture_2 = HandDet.findHandAperture(
                        frame=frame, lm_list = hand_lmlist_2, verbose=True, show_aperture=True)
                    
                    if(aperture_2 < 50):
                        msg_gripper.goal.width = 0.02
                        msg_gripper.goal.speed = 0.1
                    
                    else:
                        msg_gripper.goal.width = 0.08
                        msg_gripper.goal.speed = 0.1



                    # creating a valid angle list for the robot joint, given the extracted angles and hand aperture
                    vals = [(starting_pose[0] + coefficient*(aperture)), starting_pose[1],
                            starting_pose[2],
                            (starting_pose[3] + coefficient*((elbow_angle_3d_second-90)/100)),
                            starting_pose[4],
                            (starting_pose[5]),
                            starting_pose[6]]
                    ...
                    # publish the message
                    pub.publish(pubmsg)
                    gripper.publish(msg_gripper)
                    rate.sleep()
            except:
                print("error no encuentra manos")
