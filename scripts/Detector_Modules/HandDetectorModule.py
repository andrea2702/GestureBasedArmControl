...
class HandDetector():
    ...
    def findHands(self, frame, draw=True, return_handedness=True):
    ...
    def findHandAperture(self, frame,lm_list,verbose=False, show_aperture=False ,aperture_range: list = [0.4, 1.7]):
        '''
        ...
        thumb_cmc_lm_array = np.array([lm_list[1][1:]])[0]
        wrist_lm_array = np.array([lm_list[0][1:]])[0]
        lower_palm_midpoint_array = (thumb_cmc_lm_array + wrist_lm_array) / 2

        index_mcp_lm_array = np.array([lm_list[5][1:]])[0]
        pinky_mcp_lm_array = np.array([lm_list[5][1:]])[0]
        ...
        index_tip_array = np.array([lm_list[8][1:]])[0]
        middle_tip_array = np.array([lm_list[12][1:]])[0]
        ring_tip_array = np.array([lm_list[16][1:]])[0]
        pinky_tip_array = np.array([lm_list[20][1:]])[0]
        ...
        if(handness_list is not None):
            ...
            if len(hand_lmlist) > 0:
                frame, aperture= detector.findHandAperture(
                    frame=frame,lm_list = hand_lmlist, verbose=False, show_aperture=True)
            
            print("apertura mano1 : %f", aperture)
            ...
            try:
                ...
                if len(hand_lmlist) > 0:
                    frame, aperture_2= detector.findHandAperture(
                        frame=frame, lm_list = hand_lmlist, verbose=False, show_aperture=True)  
                
                print("apertura mano2 : %f", aperture_2)
            except:
                print("Solo hay una mano")
        else:
            print("no manos")
